import { type CLusterObj, type nestedObj } from '../../types/types'

// ~----------------------------------------Modal-------------------------------------------------->
export const makeModal = (obj: CLusterObj, type: string): string => {
  ...
  Object.keys(obj).forEach((key: any) => {
    ...
    if (typeof obj[key] === 'object') subLists.push(recursiveList(obj[key], key))
      ...
      li.innerText = `${humanReadable(key)}: ${obj[key] as string}`
      ul.appendChild(li)
    }
  })
...
const recursiveList = (obj: nestedObj, key?: any): HTMLElement => {
  const subList = document.createElement('ul')
  const smallHeader = document.createElement('div')
  smallHeader.className = 'modalHeader'
  if (key !== undefined) smallHeader.innerText = `${humanReadable(key)}`
  ...
  if (Array.isArray(obj) && obj.length === 1 && typeof obj[0] !== 'object') {
    ...
    singleLi.innerText = `${humanReadable(key)}: ${obj[0] as string}`
  ...
  Object.keys(obj).forEach((k: any) => { // fix typing here
    ...
    if (typeof obj[k] === 'object') subList.appendChild(recursiveList(obj[k]))
      ...
      Array.isArray(obj) ? li.innerText = `${obj[k] as string}` : li.innerText = `${humanReadable(k)}: ${obj[k] as string}`
...
export const humanReadable = (name: string): string => {
  const words = name.match(/[A-Za-z][a-z]*/g)
  return words?.map(capitalize).join(' ') ?? ''
}
export const capitalize = (word: string): string => word.charAt(0).toUpperCase() + word.substring(1)

// ~---------------------------------------Window Helper------------------------------------------->
// helper function to prevent benign error message created when viewing larger clusters
export const windowHelper = (): void => {
